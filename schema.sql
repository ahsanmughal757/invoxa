...
CREATE TABLE IF NOT EXISTS "public"."notifications" (
    ...
    CONSTRAINT "notifications_target_check" CHECK ((("organization_id" IS NOT NULL) OR ("recipient_user_id" IS NOT NULL)))
);
...
CREATE INDEX "idx_clients_user_id" ON "public"."clients" USING "btree" ("clerk_user_id");
...
CREATE POLICY "Allow the invited members to update the status of their respect" ON "public"."organization_invites" FOR UPDATE USING ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));



CREATE POLICY "Authenticated User can access their own invoices " ON "public"."invoices" FOR SELECT USING ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));
...
CREATE POLICY "Authenticated users can access profiles" ON "public"."profiles" FOR SELECT USING ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));



CREATE POLICY "Authenticated users can create notifications" ON "public"."notifications" FOR INSERT WITH CHECK ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));



CREATE POLICY "Authenticated users can manage thier expenses" ON "public"."expenses" USING ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));



CREATE POLICY "Authenticated users can update notification" ON "public"."notifications" FOR UPDATE USING ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));



CREATE POLICY "Authenticated users can update organizations" ON "public"."organizations" FOR UPDATE USING ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));
...
CREATE POLICY "Enable read access for organization members" ON "public"."organization_invites" FOR SELECT USING ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));



CREATE POLICY "Enable read access for user notifications" ON "public"."notifications" FOR SELECT USING ((("auth"."jwt"() ->> 'sub'::"text") IS NOT NULL));
...
CREATE POLICY "Users can manage payments in their own organizations or persona" ON "public"."payments" USING (((EXISTS ( SELECT 1
   FROM ((("public"."payments" "p"
     JOIN "public"."invoices" "i" ON (("p"."invoice_id" = "i"."id")))
     JOIN "public"."org_members" "om" ON (("i"."org_id" = "om"."org_id")))
     JOIN "public"."profiles" "pr" ON (("om"."user_id" = "pr"."id")))
  WHERE (("p"."id" = "payments"."id") AND ("pr"."clerk_user_id" = ("auth"."uid"())::"text")))) OR ("user_id" IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
