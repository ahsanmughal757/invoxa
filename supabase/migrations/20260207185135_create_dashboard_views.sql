...
SELECT
    ...
    CASE
        ...
        WHEN i.due_date < CURRENT_DATE
             AND i.paid_amount < i.total THEN 'overdue'
        WHEN i.paid_amount > 0
             AND i.paid_amount < i.total THEN 'partially_paid'
        ELSE i.status
...
DROP VIEW IF EXISTS public.v_revenue_summary;
...
SELECT
    ...
    SUM(
        CASE
            WHEN i.status = 'paid' THEN p.amount
            ELSE 0
        END
    ) AS collected_current_month,
