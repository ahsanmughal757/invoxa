...
export default function MembersManagementPage() {
  ...
  const handleRemoveMember = async () => {
    ...
    try {
      ...
      toast.success(isSelfRemoval ? "Left organization successfully" : "Member removed successfully");
      await loadMembers();
    } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-900">No Organization</h2>
          <p className="text-gray-600 mt-2">Please select or create an organization first.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              variant="ghost"
                              size="sm"
                              disabled={actionLoading?.startsWith(`remove-${member.user_id}`)}
                            >
                              {actionLoading?.startsWith(`remove-${member.user_id}`) ? (
      ...
      <RemoveMemberModal
        ...
        isLastOwner={selectedMember ? 
          members.filter(m => m.role === "owner").length === 1 && selectedMember.role === "owner" 
          : false}
      />
    </div>
  );
